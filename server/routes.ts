...
// Register test receipt route globally so it's always available
import { sendReceiptEmail } from "./email";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  whatsappBotService.initialize().catch(console.error);
  // WhatsApp receipt polling API for AchekBot
  app.get("/api/bot/receipts", async (req, res) => {
    try {
      // You can customize this to return only new/unprocessed receipts for the bot
      if (typeof storage.getOrders === "function") {
        const orders = storage.getOrders();
        res.json({ success: true, receipts: orders });
      } else {
        res.json({ success: false, receipts: [] });
      }
    } catch (error) {
      res.status(500).json({ success: false, error });
    }
  ...
  // Receipt email and WhatsApp onboarding after payment
  // WhatsApp receipt automation: log to local store for bot polling
  // WhatsApp receipts module will be added later
  app.post("/api/send-receipt", async (req, res) => {
    try {
      ...
      // Add WhatsApp receipt to local store for bot
      // WhatsApp receipt logging will be added with bot implementation
      // WhatsApp admin notification (stub)
      // You can integrate Twilio or WhatsApp Business API here
      const adminWhatsapp = "08104040841";
      const adminMsg = `New order received: ${name}, ${email}, ${service} (${pkg}), ₦${amount}. Contact client at ${whatsapp}.`;
      // TODO: Integrate WhatsApp API to send adminMsg to adminWhatsapp
      // WhatsApp onboarding message for user
      const whatsappMsg = `Hello ${name}, thank you for your payment for ${service} (${pkg}). Our team will contact you here to begin your project onboarding!`;
      const whatsappLink = `https://wa.me/${whatsapp.replace(/[^0-9]/g,"")}?text=${encodeURIComponent(whatsappMsg)}`;
      res.json({ success: emailSent && adminNotified, whatsappLink });
    } catch (error) {
