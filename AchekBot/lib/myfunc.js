// Project intake state helpers
const path = require('path');
const intakeStatePath = path.join(__dirname, '../data/intakeState.json');

function setIntakeState(userId, state) {
    let data = {};
    try {
        data = JSON.parse(fs.readFileSync(intakeStatePath, 'utf-8'));
    } catch (e) {
        data = {};
    }
    data[userId] = state;
    fs.writeFileSync(intakeStatePath, JSON.stringify(data, null, 2));
}
...
function clearIntakeState(userId) {
    let data = {};
    try {
        data = JSON.parse(fs.readFileSync(intakeStatePath, 'utf-8'));
    } catch (e) {
...
exports.setIntakeState = setIntakeState;
exports.getIntakeState = getIntakeState;
exports.clearIntakeState = clearIntakeState;
// Service request state helpers
const pendingServicePath = path.join(__dirname, '../data/pendingService.json');

function setPendingService(userId) {
    let data = {};
    try {
        data = JSON.parse(fs.readFileSync(pendingServicePath, 'utf-8'));
    } catch (e) {
        data = {};
    }
    data[userId] = true;
    fs.writeFileSync(pendingServicePath, JSON.stringify(data, null, 2));
}

function clearPendingService(userId) {
    let data = {};
    try {
        data = JSON.parse(fs.readFileSync(pendingServicePath, 'utf-8'));
    } catch (e) {
...
function isPendingService(userId) {
    let data = {};
    try {
        data = JSON.parse(fs.readFileSync(pendingServicePath, 'utf-8'));
    } catch (e) {
...
exports.setPendingService = setPendingService;
exports.clearPendingService = clearPendingService;
exports.isPendingService = isPendingService;
// Onboarding helpers
const userDataPath = path.join(__dirname, '../data/userGroupData.json');
...
// const path = require('path') // Duplicate, remove
